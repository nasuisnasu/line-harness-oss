...
/**
 * テストの管理画面（生徒一覧 ／ 問題集）── 4種で共有
 *
 * 単語・熟語・4択・講座は**生徒から見て別のテスト**（リッチメニューも別枠）。
 * 管理画面も分ける。ここはその共通の中身で、`match` で対象の問題集を絞る。
 *
 *   /grammar       4択（英文法マスター講座）
 *   /idiom         熟語（ターゲット1000・速読英熟語・パス単）
 *   /course-test   講座（文法講座テスト）
 *   /vocab         単語（別実装）
 *
...
import GrammarStudentDetailPanel from '@/app/grammar/student-detail'
import GrammarBooksPanel from '@/app/grammar/books'
import type { GrammarBookSummary } from '@/lib/api'
...
export default function GrammarAdmin({
  ...
  // どの問題集がこの画面の対象か。slug で引くので、本が増えても勝手に入る
  useEffect(() => {
    ...
    api.grammar
      ...
      .then((res) => {
        if (!alive) return
        const ids = (res.books ?? [])
          .filter((b: GrammarBookSummary) => match(b.slug))
          .map((b: GrammarBookSummary) => b.id)
        setBookIds(ids)
      })
      .catch(() => setBookIds([]))
    return () => {
  ...
  const load = useCallback(async () => {
    if (!selectedAccount || bookIds === null) return
    setLoading(true)
    setError('')
    try {
      // lineAccountId は必ず渡す。渡さないと複数OAの生徒が混ざる。
      // book_ids を渡さないと、生徒が最後に解いた1冊（別のテスト）が出る。
      const res = await api.grammar.students({
